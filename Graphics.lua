...
function Graphics:updatePlayersHistogramCanvas(canvas, t, selected)
  ...
  local lineWidth = (width-40)/50 -- 50 градаций по рейтингу
  local thinkess = lineWidth - 2

  local lineHeight = (height-40)*3/#t -- количество игроков
  --local percentW = math.floor(#t/10)
  ...
  love.graphics.setColor(0.5,0.5,1)                                                                             -- rektangles histogram
  for i=1,50 do
    amount = Game.stat.playersByELO50[i]
  ...
  love.graphics.setColor(1,1,1)                                               -- рейтинг
  for i=0,49 do
